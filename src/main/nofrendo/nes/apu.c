...
static const uint8 vbl_length[32] = {5, 127, 10, 1, 19, 2,  40, 3,  80, 4,  30, 5,  7, 6,  13, 7,
                                     6, 8,   12, 9, 24, 10, 48, 11, 96, 12, 36, 13, 8, 14, 16, 15};

/* frequency limit of rectangle channels */
static const int freq_limit[8] = {0x3FF, 0x555, 0x666, 0x71C, 0x787, 0x7C1, 0x7E0, 0x7F0};

/* noise frequency lookup table */
static const int noise_freq[16] = {4, 8, 16, 32, 64, 96, 128, 160, 202, 254, 380, 508, 762, 1016, 2034, 4068};

/* DMC transfer freqs */
static const int dmc_clocks[16] = {428, 380, 340, 320, 286, 254, 226, 214, 190, 160, 142, 128, 106, 85, 72, 54};
...
void apu_fc_advance(int cycles) {
  ...
  if (apu.fc.cycles >= int_period) {
    ...
    if ((apu.fc.state & 0xC0) == 0) {
      ...
      if (!apu.fc.disable_irq) nes6502_irq();
    }
  }
}

void apu_setcontext(const apu_t *src) { MESSAGE_ERROR("%s: Not implemented!\n", __func__); }

void apu_getcontext(apu_t *dest) { MESSAGE_ERROR("%s: Not implemented!\n", __func__); }

static void apu_build_luts(int num_samples) {
  /* lut used for enveloping and frequency sweeps */
  for (int i = 0; i < 16; i++) apu.decay_lut[i] = num_samples * (i + 1);

  /* used for note length, based on vblank and size of audio buffer */
  for (int i = 0; i < 32; i++) apu.vbl_lut[i] = vbl_length[i] * num_samples;

  /* triangle wave channel's linear length table */
  for (int i = 0; i < 128; i++) apu.trilength_lut[i] = (int)(0.25 * i * num_samples);
}
...
#define APU_MAKE_RECTANGLE(ch)                                                                                \
  static inline int apu_rectangle_##ch(void) {                                                                \
    ...
    APU_VOLUME_DECAY(apu.rectangle[ch].output_vol);                                                           \
                                                                                                              \
    if (!apu.rectangle[ch].enabled || apu.rectangle[ch].vbl_length == 0) return apu.rectangle[ch].output_vol; \
                                                                                                              \
    /* vbl length counter */                                                                                  \
    if (!apu.rectangle[ch].holdnote) apu.rectangle[ch].vbl_length--;                                          \
                                                                                                              \
    ...
    if (apu.rectangle[ch].freq < 8 ||                                                                         \
        (false == apu.rectangle[ch].sweep_inc && apu.rectangle[ch].freq > apu.rectangle[ch].freq_limit))      \
    ...
    if (apu.rectangle[ch].accum >= 0) return apu.rectangle[ch].output_vol;                                    \
                                                                                                              \
...
static inline int apu_triangle(void) {
  ...
  if (!apu.triangle.enabled || apu.triangle.vbl_length == 0) goto output;

  if (apu.triangle.counter_started) {
    if (apu.triangle.linear_length > 0) apu.triangle.linear_length--;
    if (apu.triangle.vbl_length && false == apu.triangle.holdnote) apu.triangle.vbl_length--;
  }
...
static inline int apu_noise(void) {
  ...
  if (!apu.noise.enabled || apu.noise.vbl_length == 0) goto output;

  /* vbl length counter */
  if (!apu.noise.holdnote) apu.noise.vbl_length--;
  ...
  if (apu.noise.accum >= 0) goto output;
...
void apu_write(uint32 address, uint8 value) {
  ...
  switch (address) {
    ...
    case APU_SMASK:
      ...
      if (value & 0x10) {
        if (apu.dmc.dma_length == 0) apu_dmcreload();
      }
      else {
...
int apu_getopt(apu_option_t n) { return apu.options[n]; }
...
void apu_reset(void) {
  ...
  for (uint32 addr = 0x4000; addr <= 0x4013; addr++) apu_write(addr, 0);
  ...
  if (apu.ext && apu.ext->reset) apu.ext->reset();
}
...
void apu_setext(const apuext_t *ext) { apu.ext = ext; }
