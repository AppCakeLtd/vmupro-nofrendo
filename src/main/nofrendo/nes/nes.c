...
void nes_emulate(bool draw) {
  ...
  while (nes.scanline < nes.scanlines_per_frame) {
    ...
    ppu_renderline(nes.vidbuf, nes.scanline, draw, nes.builtPalette);

    if (nes.scanline == 241) {
      elapsed_cycles += nes6502_execute(6);
      if (nes.ppu->ctrl0 & PPU_CTRL0F_NMI) nes6502_nmi();

      if (nes.mapper->vblank) nes.mapper->vblank(&nes);
    }
  ...
  if (draw && nes.blit_func) nes.blit_func(nes.vidbuf);
...
void nes_reset(bool hard_reset) {
  if (hard_reset) {
    if (nes.cart->chr_ram_banks > 0) memset(nes.cart->chr_ram, 0, nes.cart->chr_ram_banks * ROM_CHR_BANK_SIZE);
    if (nes.cart->prg_ram_banks > 0) memset(nes.cart->prg_ram, 0, nes.cart->prg_ram_banks * ROM_PRG_BANK_SIZE);
  }
...
nes_t* nes_init(nes_type_t system, int sample_rate, bool stereo, const char* fds_bios) {
  ...
  /* memory */
  nes.mem = mem_init_();
  if (NULL == nes.mem) goto _fail;

  /* cpu */
  nes.cpu = nes6502_init(nes.mem->pages);
  if (NULL == nes.cpu) goto _fail;

  /* ppu */
  nes.ppu = ppu_init();
  if (NULL == nes.ppu) goto _fail;

  /* apu */
  nes.apu = apu_init(sample_rate, stereo);
  if (NULL == nes.apu) goto _fail;
