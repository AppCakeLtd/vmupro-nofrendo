...
rom_t *rom_loadmem(uint8 *data, size_t size) {
  ...
  if (!memcmp(data, ROM_NES_MAGIC, 4)) {
    ...
    if (header->reserved2 != 0) rom.mapper_number &= 0x0F;
    ...
    while (entry->crc && entry->crc != rom.checksum) entry++;
    ...
    size_t expected_size =
        data_offset + (rom.prg_rom_banks * ROM_PRG_BANK_SIZE) + (rom.chr_rom_banks * ROM_CHR_BANK_SIZE);
    ...
    if (rom.chr_rom_banks > 0) rom.chr_rom = data + data_offset + (rom.prg_rom_banks * ROM_PRG_BANK_SIZE);
  }
  ...
  MESSAGE_INFO(
      "ROM: Mapper: %d, PRG:%dK, CHR:%dK, Flags: %c%c%c%c\n",
      rom.mapper_number,
      rom.prg_rom_banks * 8,
      rom.chr_rom_banks * 8,
      (rom.vertical) ? 'V' : 'H',
...
rom_t *rom_loadfile(const char *filename) {
  ...
  if (rom.system == SYS_UNKNOWN) {
    if (strstr(filename, "(E)") || strstr(filename, "(Europe)") || strstr(filename, "(A)") ||
...
void rom_free(void) {
  if (rom.type != ROM_TYPE_INVALID) {
    if (rom.free_data_ptr) free(rom.data_ptr);
