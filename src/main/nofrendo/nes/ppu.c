...
INLINE void ppu_oamdma(uint8 value) {
  ...
  for (size_t i = 0; i < 256; ++i) ppu.oam[ppu.oam_addr++] = mem_getbyte(cpu_address++);
...
uint8 ppu_read(uint32 address) {
  ...
  switch (address & 0x2007) {
    case PPU_STAT:
      ...
      if (ppu.strikeflag) {
        if (nes6502_getcycles() >= ppu.strike_cycle) value |= PPU_STATF_STRIKE;
      }
  ...
  switch (address & 0x2007) {
    ...
    case PPU_VDATA:
      if (ppu.vaddr < 0x3F00) {
        ...
        if (0 == (ppu.vaddr & 0x0F)) {
          for (int i = 0; i < 8; i++) ppu.palette[i << 2] = (value & 0x3F) | BG_TRANS;
        }
        else if (ppu.vaddr & 3) {
...
int ppu_getopt(ppu_option_t n) { return ppu.options[n]; }

void ppu_setlatchfunc(ppu_latchfunc_t func) { ppu.latchfunc = func; }

void ppu_setvreadfunc(ppu_vreadfunc_t func) { ppu.vreadfunc = func; }
...
INLINE void ppu_renderbg(uint8 *vidbuf) {
  ...
  uint32 attrib        = PPU_MEM_READ(attrib_addr);
  attrib_addr++;
  ...
  for (int tile_num = 0; tile_num < 33; tile_num++) {
    ...
    if (ppu.latchfunc && (tile_index == 0xFD || tile_index == 0xFE)) ppu.latchfunc(ppu.bg_base, tile_index);
    ...
    if (0 == (x_tile & 1)) /* check every 2 tiles */
    {
      if (0 == (x_tile & 3)) /* check every 4 tiles */
      {
        ...
        attrib = PPU_MEM_READ(attrib_addr);
        attrib_addr++;
      }
...
INLINE void ppu_renderoam(uint8 *vidbuf, int scanline, bool draw) {
  ...
  for (int sprite_num = 0, count = 0; sprite_num < 64; sprite_num++) {
    ...
    /* Handle $FD/$FE magic tile CHR-ROM switching (MMC2/MMC4) */
    if (ppu.latchfunc && (tile_index == 0xFD || tile_index == 0xFE)) ppu.latchfunc(sprite_offset, tile_index);
    ...
    /* Fetch tile and draw it */
    draw_oamtile(
        vidbuf + sprite->x_loc, sprite->attr, get_patpix(tile_addr), ppu.palette + 16 + ((sprite->attr & 3) << 2)
    );
  ...
  if (draw && !ppu.left_obj_on) memcpy(vidbuf, &savecol, 8);
}
...
static void draw_sprite(uint8 *bmp, int x, int y, uint8 tile_num, uint8 attrib) {
  ...
  if (16 == height) tile_addr = ((tile_num & 1) << 12) | ((tile_num & 0xFE) << 4);
  /* else just use the offset from $2000 */
  else
    tile_addr = ppu.obj_base + (tile_num << 4);

  for (line = 0; line < height; line++) {
    if (line == 8) tile_addr += 8;
