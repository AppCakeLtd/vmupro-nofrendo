...
static const mem_read_handler_t read_handlers[] =
    {{0x2000, 0x3FFF, ppu_read}, {0x4000, 0x4015, apu_read}, {0x4016, 0x4017, input_read}, LAST_MEMORY_HANDLER};
...
uint8 mem_getbyte(uint32 address) {
  ...
  if (flags & MEM_PAGE_HAS_READ_HANDLER) {
    for (mem_read_handler_t *mr = mem.read_handlers; mr->handler != NULL; mr++) {
      if (address >= mr->min_range && address <= mr->max_range) return mr->handler(address);
    }
  }
...
uint32 mem_getword(uint32 address) { return mem_getbyte(address + 1) << 8 | mem_getbyte(address); }
...
void mem_reset(void) {
  ...
  if (mapper) {
    for (size_t i = 0; i < 4 && mapper->mem_read[i].handler; i++) *mem_r++ = mapper->mem_read[i];
    for (size_t i = 0; i < 4 && mapper->mem_write[i].handler; i++) *mem_w++ = mapper->mem_write[i];
  }

  // NES hardware handlers
  for (size_t i = 0; i < MEM_HANDLERS_MAX - 5 && read_handlers[i].handler; i++) *mem_r++ = read_handlers[i];
  for (size_t i = 0; i < MEM_HANDLERS_MAX - 5 && write_handlers[i].handler; i++) *mem_w++ = write_handlers[i];
  ...
  for (mem_read_handler_t *mr = mem.read_handlers; mr < mem_r; mr++) {
    for (int i = mr->min_range; i < mr->max_range; i++) mem.flags[i >> MEM_PAGESHIFT] |= MEM_PAGE_HAS_READ_HANDLER;
  }
  for (mem_write_handler_t *mw = mem.write_handlers; mw < mem_w; mw++) {
    for (int i = mw->min_range; i < mw->max_range; i++) mem.flags[i >> MEM_PAGESHIFT] |= MEM_PAGE_HAS_WRITE_HANDLER;
  }
}
