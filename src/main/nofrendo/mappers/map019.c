...
static uint8 ram_read(uint32 address) {
  ...
  if (map019->address & RAM_AUTO_INCREMENT) map019->address = (map019->address + 1) | RAM_AUTO_INCREMENT;
  return temp;
}
...
static void map_hblank(nes_t *nes) {
  ...
  if (map019->sound.changed) sound_prepare();
}
...
static void map_init(rom_t *cart) {
  if (!map019) map019 = malloc(sizeof(map019_t));
